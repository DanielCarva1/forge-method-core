...
- MCP stdio remains read-only by policy: any allowlist containing a mutating tool now fails startup even when the new registry is present. This prevents identity from being discarded into a replayable subprocess path while replay integration, constrained in-process handoff, kernel pre-WAL enforcement, principal propagation, and operation-wide atomicity remain subsequent checkpoints.
...
- **Source migration:** P4b.1a added fields to the public Rust `McpServerConfig`, so downstream struct literals must supply them or use `McpServerConfig::default_read_only()`. Legacy read-only wire payloads remain compatible because new optional attestation fields are omitted when `None`. Existing custom MCP allowlists must remove `policy: mutate`; a principal registry does not enable mutation.
