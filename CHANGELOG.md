...
- MCP stdio remains read-only by policy: any allowlist containing a mutating tool fails startup even when both the registry and in-process executor are present. P4b.2a removes the forgeable subprocess handoff and P4b.2b proves dormant replay-bound late Admission, but provenance, effect/replay crash recovery, and operation-wide atomicity remain subsequent checkpoints.
