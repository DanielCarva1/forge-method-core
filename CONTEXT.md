...
`mcp`, `claim`, `contract`, and `memory` CLI help paths project their usage
lines from the same seam. Command-tree help and unknown-subcommand hints use
`CommandSpec` projection helpers for local usage lines, concrete subcommand
names, and full subcommand usage lookup instead of re-implementing
