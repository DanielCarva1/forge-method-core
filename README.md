...
The active source line is **`0.12.0-alpha.31`**. `Cargo.toml` is the single
...
|---|---|---|
| **Source checkpoint** | Workspace package SemVer `0.12.0-alpha.31` plus the exact Git commit/working-tree state. A dirty checkout is not immutable. | `git rev-parse HEAD`, `git status --short`, and `[workspace.package].version` in `Cargo.toml` |
