...
$installedUpdateSkill = Join-Path $HOME ".agents\skills\forge-update\SKILL.md"
...
if (-not (Test-Path -LiteralPath $installedUpdateSkill)) {
  throw "Installed update skill not found: $installedUpdateSkill"
}
...
$installedGameBriefText = Get-Content -Raw -Path (Join-Path $exampleTmp ".forge-method\artifacts\install-game-brief.md")
Assert-Contains $installedGameBriefText "mda_trace:" "installed game brief artifact"
Assert-Contains $installedGameBriefText "proof_or_playtest: player can move through one room and collect one item" "installed game brief artifact"
...
Assert-Contains $installedProjectGuideText "First question: give me the whole picture first: who is it for, what should change for them, what is fixed or out, what is still open, and what visible or operational proof should close discovery?" "installed project first answer guide output"
