...
Assert-Contains $projectGuideText "First question: give me the whole picture first: who is it for, what should change for them, what is fixed or out, what is still open, and what visible or operational proof should close discovery?" "project first answer guide output"
...
$smokeGameBriefText = Get-Content -Raw -Path (Join-Path $tmp ".forge-method\artifacts\smoke-game-brief.md")
Assert-Contains $smokeGameBriefText "mda_trace:" "smoke game brief artifact"
Assert-Contains $smokeGameBriefText "proof_or_playtest: player can move through one room and collect one item" "smoke game brief artifact"
