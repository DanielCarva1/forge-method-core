...
// Re-export the sigstore trust policy / certificate / bundle helpers at
// the crate root so the host-adapter verification functions in `lib.rs`
// keep resolving after the helpers moved into `crypto_sigstore`. Uses a
// wildcard re-export (same pattern as `host_adapter_types`) because the
// module is large and many of its items are consumed by multiple
// verifiers; this can be tightened to explicit names in a later pass if
// desired.
#[allow(clippy::wildcard_imports)]
pub(crate) use crypto_sigstore::*;
// Re-export the SLSA provenance / transparency log / ed25519 signature
// helpers at the crate root so the host-adapter provenance, sigstore
// bundle subject, and DSSE in-toto subject verification functions in
// `lib.rs` keep resolving after the helpers moved into
// `crypto_slsa_transparency`. Uses wildcard re-export (same pattern as
// `host_adapter_types` and `crypto_sigstore`) for consistency; the module
// is small enough that tightening to explicit names is a trivial future
// refinement.
#[allow(clippy::wildcard_imports)]
