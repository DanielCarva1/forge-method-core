...
// `host_command`. The `host_command` builder and `HostCommandMetadata`
// are now consumed only by `host_adapter_manifest`, which imports them
// directly from `crate::host_command`.
...
// Only `RuntimeKind` is still consumed directly by `lib.rs` (host-adapter
// manifest + distribution policy). Every other contract document type and
// every `validate_*` function lives in the `validate` module now.
