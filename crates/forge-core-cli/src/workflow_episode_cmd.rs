...
use forge_core_kernel::PostBuildVerifyEpisodeApplyRequest;
use serde::Deserialize;
use std::path::PathBuf;

const COMMAND: &str = "workflow.episode.apply";
const MAX_EPISODE_APPLY_INPUT_BYTES: u64 = 256 * 1024;
...
fn read_input(path: &PathBuf) -> Result<EpisodeApplyInput, String> {
    let bytes =
        crate::io_util::read_regular_file_no_follow_bounded(path, MAX_EPISODE_APPLY_INPUT_BYTES)
            .map_err(|error| {
                format!(
                    "invalid episode apply input: {error}; maximum {} bytes",
                    MAX_EPISODE_APPLY_INPUT_BYTES
                )
            })?;
