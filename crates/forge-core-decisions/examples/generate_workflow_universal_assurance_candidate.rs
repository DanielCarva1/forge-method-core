...
const ASSURANCE_OPERATIONS_BATCH_PATH: &str =
...
const OVERLAY_PATH: &str = "contracts/policies/workflow-universal-assurance-overlay-v0.yaml";
const REVIEW_SUBJECT_PATH: &str =
    "contracts/migration/workflow-universal-assurance-review-subject-v0.yaml";
...
const ADVERSARIAL_EVIDENCE_PATH: &str =
    "contracts/evidence/workflow-universal-assurance-adversarial-v0.yaml";
const SHADOW_REPORT_PATH: &str =
    "contracts/evidence/workflow-universal-assurance-shadow-report-v0.yaml";
...
const PREDECESSOR_REGISTRY_SHA256: &str =
    "sha256:508ed9b2590bdf4042138fcce5585ad1c8ac200560a1a915698499a151da6fc4";
...
const BASE_WORKFLOW_IDS: [&str; 42] = [
    "adversarial-review",
    "architecture",
    "atdd-plan",
    "build-story",
...
const OVERLAY_WORKFLOW_IDS: [&str; 1] = ["guidance-engine"];
...
fn parse_mode() -> Result<Mode, Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        _ => Err(error(
            "usage: cargo run -p forge-core-decisions --example generate_workflow_universal_assurance_candidate -- (--write|--check)",
        )),
    }
}
...
fn generate(root: &Path, mode: Mode) -> Result<Vec<GeneratedArtifact>, Box<dyn Error>> {
    ...
    eprintln!(
        "compatibility debt: frozen predecessor evaluators may list accepted evidence tuples that the trusted authority matrix cannot mint; every frozen evaluator retains at least one usable authorized tuple, while every universal-assurance overlay tuple is exact"
    );
    // P7b.2 cannot rewrite the frozen predecessor policy set.
    ...
    let runtime_bundle = WorkflowGovernanceBundleDocument {
        ...
        workflow_governance_bundle: WorkflowGovernanceBundle {
            id: id("bundle.workflow-governance.universal-assurance-candidate-v0"),
    ...
    let assurance_operations_batch: WorkflowMigrationBatchDocument =
        read_yaml(&root.join(ASSURANCE_OPERATIONS_BATCH_PATH))?;
...
fn validate_exact_policy_set(
    ...
    for policy in policies {
        let expected_policy_id = if policy.compatibility_workflow_id.0 == "guidance-engine" {
            "policy.workflow.universal-assurance".to_owned()
        } else {
            format!("policy.workflow.{}", policy.compatibility_workflow_id.0)
        };
...
fn validate_overlay_header(
    ...
    if overlay.schema_version != WORKFLOW_GOVERNANCE_SCHEMA_VERSION
        || overlay.workflow_governance_policy_overlay.id.0
            != "overlay.workflow-governance.universal-assurance-v0"
        ...
        || overlay.workflow_governance_policy_overlay.base_bundle_id.0
            != "bundle.workflow-governance.agent-native-continuity-v0"
    {
        return Err(error(
            "universal-assurance overlay header or exact predecessor binding is invalid",
        ));
    }
    Ok(())
}
...
fn validate_composition(policies: &[WorkflowGovernancePolicy]) -> Result<(), Box<dyn Error>> {
    if policies.len() != 43 {
        return Err(error(format!(
            "candidate runtime must contain exactly 43 policies, found {}",
            policies.len()
        )));
    }
    ...
    let workflow_ids = policies
        ...
        .map(|policy| policy.compatibility_workflow_id.0.as_str())
        .collect::<BTreeSet<_>>();
    if policy_ids.len() != 43 || workflow_ids.len() != 43 {
        return Err(error(
...
fn validate_registry_sentinel(root: &Path) -> Result<(), Box<dyn Error>> {
    let registry_bytes = std::fs::read(root.join(PREDECESSOR_REGISTRY_PATH)).map_err(|cause| {
        error(format!(
            "embedded agent-native-continuity registry is unavailable at {PREDECESSOR_REGISTRY_PATH}: {cause}"
        ))
    })?;
    let found = sha256(&registry_bytes);
    if found != PREDECESSOR_REGISTRY_SHA256 {
        return Err(error(format!(
            "universal-assurance generation cannot change admitted predecessor registry bytes: expected {PREDECESSOR_REGISTRY_SHA256}, found {found}"
        )));
    }
    Ok(())
}
...
fn validate_predecessor_inputs(
    ...
    if release.release_id.0 != "workflow-governance.release.agent-native-continuity-v0"
        || release.release_version != "0.4.0"
        || release.batches.len() != 4
        || release.batches[3].embedded_ref.0 != BASE_BATCH_PATH
        || release.batches[3].expected_digest
            != workflow_migration_batch_digest(batch).map_err(error)?
        || predecessor_batch.id.0 != "workflow-governance.batch.agent-native-continuity-v0"
        || predecessor_batch.policies.len() != 9
        || release.workflow_entries.len() != 110
    {
        return Err(error(
            "agent-native-continuity artifacts are not the exact expected predecessor",
        ));
    }
...
fn build_predecessor_history(root: &Path) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    if documents.len() != 4 {
        return Err(error(
            "predecessor history must contain exactly four records",
        ));
    }
    ...
    let previous = documents[3].workflow_governance_receipt.clone();
    ...
    upgrade.admission_proof.proof_id =
        id("proof.workflow-governance.release-admission.agent-native-continuity-v0");
    upgrade
        .admission_proof
    ...
    record.record_id = id("wglr-p7b2-agent-native-continuity-frozen-history");
    record.sequence = 5;
...
fn build_review_subject(
    ...
    let predecessor = predecessor_registry
        ...
        .last()
        .ok_or_else(|| error("predecessor registry must contain a release"))?;
    if predecessor.release.release_id.0 != "workflow-governance.release.agent-native-continuity-v0"
        || predecessor.runtime_bundle.identity.bundle_id.0
            != "bundle.workflow-governance.agent-native-continuity-v0"
    {
        return Err(error(
            "predecessor registry tail is not agent-native-continuity",
        ));
    }
    ...
    Ok(WorkflowBehavioralReviewSubjectDocument {
        ...
        workflow_behavioral_review_subject: WorkflowBehavioralReviewSubject {
            id: id("workflow-review-subject.universal-assurance-v0"),
            authority: WorkflowBehavioralReviewSubjectAuthority::CandidateOnly,
            overlay: WorkflowBehavioralArtifactReference {
                id: id("overlay.workflow-governance.universal-assurance-v0"),
            ...
            baseline_history: WorkflowBehavioralArtifactReference {
                id: id("history.workflow-governance.agent-native-continuity-v0"),
            ...
            proposed_batch: WorkflowBehavioralProposedBatchSubject {
                batch_id: id("workflow-governance.batch.universal-assurance-v0"),
                batch_version: "0.5.0".to_owned(),
            ...
            proposed_release: WorkflowBehavioralProposedReleaseSubject {
                ...
                release_id: id("workflow-governance.release.universal-assurance-v0"),
...
fn build_overlay_batch(
    ...
    if previous_batch_digest != workflow_migration_batch_digest(base_batch).map_err(error)? {
        return Err(error(
            "previous batch digest is not the exact agent-native-continuity batch digest",
        ));
    }
    Ok(WorkflowMigrationBatchDocument {
        ...
        workflow_migration_batch: WorkflowMigrationBatch {
            id: id("workflow-governance.batch.universal-assurance-v0"),
            batch_version: "0.5.0".to_owned(),
...
fn build_candidate_manifest(
    ...
    release.release_id = id("workflow-governance.release.universal-assurance-v0");
    "0.5.0".clone_into(&mut release.release_version);
    ...
    release.compatibility_policy.replacement_argv = [
        ...
        "--batch-file",
        ASSURANCE_OPERATIONS_BATCH_PATH,
        "--batch-file",
        BASE_BATCH_PATH,
        "--batch-file",
        BATCH_PATH,
    ...
    .collect();
    let workflow_digests = workflow_digest_map(workflows)?;
    let overlay_policy_by_workflow =
        BTreeMap::from([("guidance-engine", id("policy.workflow.universal-assurance"))]);
...
fn validate_final_invariants(
    ...
    if quarantined != expected_quarantine
        || batch.workflow_migration_batch.policies.len() != 1
        || batch.workflow_migration_batch.workflow_bindings.len() != 1
        || runtime.workflow_governance_bundle.policies.len() != 43
        || release.batches.len() != 5
        || release.batches[0].embedded_ref.0 != GOLDEN_BATCH_PATH
        || release.batches[1].embedded_ref.0 != CORE_ASSURANCE_BATCH_PATH
        || release.batches[2].embedded_ref.0 != ASSURANCE_OPERATIONS_BATCH_PATH
        || release.batches[3].embedded_ref.0 != BASE_BATCH_PATH
        || release.batches[4].embedded_ref.0 != BATCH_PATH
    {
        return Err(error(
...
fn check_artifacts(root: &Path, artifacts: &[GeneratedArtifact]) -> Result<(), Box<dyn Error>> {
    let drift = artifacts
        ...
        .collect::<Vec<_>>();
    if drift.is_empty() {
        println!("workflow universal-assurance candidate artifacts are byte-exact");
        Ok(())
    } else {
        Err(error(format!(
            "workflow universal-assurance candidate drift:\n{}\nrun the generator with --write",
            drift.join("\n")
        )))
    }
}
...
mod tests {
    ...
    fn overlay_evaluators_have_only_authorizable_tuples() {
        ...
        .expect("exact universal-assurance overlay id");
        validate_evaluator_authority_matrix(&overlay.workflow_governance_policy_overlay.policies)
    ...
    fn composed_runtime_is_valid_and_quarantine_cannot_leak() {
        ...
        assert_eq!(runtime.workflow_governance_bundle.policies.len(), 43);
        ...
        validate_composition(&runtime.workflow_governance_bundle.policies)
            .expect("closed 43-policy composition");
