...
const CORPUS_SET_PATH: &str = "contracts/evidence/workflow-universal-assurance-corpus-set-v0.yaml";
const SHADOW_REPORT_PATH: &str =
    "contracts/evidence/workflow-universal-assurance-shadow-report-v0.yaml";
const WORKFLOW_IDS: [&str; 1] = ["guidance-engine"];
...
fn parse_mode() -> Result<Mode, Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        _ => Err(error(
            "usage: cargo run -p forge-core-decisions --example generate_workflow_universal_assurance_evidence -- (--write|--check)",
        )),
    }
}
...
fn generate(root: &Path) -> Result<Vec<GeneratedArtifact>, Box<dyn Error>> {
    ...
    let corpus_set = WorkflowBehavioralCorpusSetDocument {
        ...
        workflow_behavioral_corpus_set: WorkflowBehavioralCorpusSet {
            id: id("corpus-set.workflow-universal-assurance-v0"),
    ...
    let corpus_set_artifact = artifact(
        "corpus-set.workflow-universal-assurance-v0",
        CORPUS_SET_PATH,
        &corpus_set_bytes,
    );
    context.sources.insert(
    ...
    let report = evaluate_workflow_behavior(
        &WorkflowBehavioralReportIdentity {
            report_id: id("report.workflow-universal-assurance.shadow-v0"),
            ...
            })
    {
        return Err(error(format!(
            "derived shadow evidence is not a closed 1x7 review candidate: {:#?}",
...
fn load_context(root: &Path) -> Result<EvidenceContext, Box<dyn Error>> {
    ...
    let overlay_bytes = read_bytes(root, OVERLAY_PATH)?;
    reject_crlf(OVERLAY_PATH, &overlay_bytes)?;
    ...
    Ok(EvidenceContext {
        ...
        coverage_artifact: artifact(
            "coverage.workflow-universal-assurance-v0",
            COVERAGE_PATH,
            &coverage_bytes,
        ),
...
fn parse_frozen_baseline(bytes: &[u8]) -> Result<FrozenBaseline, Box<dyn Error>> {
    ...
    if records.len() != 5 {
        return Err(error(
            "frozen predecessor history must contain exactly five records",
        ));
    }
    let imported_record = &records[0].workflow_governance_receipt;
    let upgraded_record = &records[4].workflow_governance_receipt;
    ...
    let digest_chain_is_valid = records.windows(2).all(|pair| {
        pair[1]
            .workflow_governance_receipt
            .previous_record_digest
            .as_deref()
            == Some(pair[0].workflow_governance_receipt.record_digest.as_str())
    });
    let prior_record = &records[3].workflow_governance_receipt;
    if !digest_chain_is_valid
        || upgraded.prior_ledger_head_digest != prior_record.record_digest
...
fn coverage_policy() -> WorkflowBehavioralCoveragePolicyDocument {
    WorkflowBehavioralCoveragePolicyDocument {
        ...
        workflow_behavioral_coverage_policy: WorkflowBehavioralCoveragePolicy {
            id: id("coverage.workflow-universal-assurance-v0"),
...
fn resume_scenario(
    ...
    let checkpoint_source = artifact(
        &format!(
            "checkpoint.workflow-universal-assurance.{}.v0",
...
fn ablation_scenario(
    ...
    let retained_claims = ablated
        .workflow_governance_bundle
        .policies
        .iter()
        .find(|candidate| candidate.id == policy.id)
        .expect("ablated policy remains addressable")
        .claims
        .iter()
    ...
    evaluation
        ...
        .evidence
        .retain(|evidence| {
            evidence.claim_ref != *selected_claim && retained_claims.contains(&evidence.claim_ref)
        });
    evaluation
        .workflow_governance_evaluation
...
fn ablate_policy(
    ...
    let selected_claim = policy
        ...
        .find(|claim| {
            ...
            let structurally_redundant_lenses = !lens_aware
                || (!claim.assurance_lenses.is_empty()
                    && claim.assurance_lenses.iter().all(|lens| {
                        policy.claims.iter().any(|candidate| {
                            candidate.id != claim.id && candidate.assurance_lenses.contains(lens)
                        })
                    }));
            closed && structurally_redundant_lenses
        })
        .ok_or_else(|| {
            error(format!(
                "policy {} has no structurally valid load-bearing ablation claim",
                policy.id.0
            ))
        })?
        .clone();
...
fn ablated_path(workflow_id: &str) -> String {
    format!("contracts/workflow-governance/ablated-universal-assurance-{workflow_id}-v0.yaml")
}

fn checkpoint_path(workflow_id: &str) -> String {
    format!(
        "contracts/evidence/workflow-universal-assurance-{workflow_id}-resume-checkpoint-v0.yaml"
    )
}
...
fn reject_crlf(path: &str, bytes: &[u8]) -> Result<(), Box<dyn Error>> {
    if bytes.windows(2).any(|pair| pair == b"\r\n") {
        return Err(error(format!(
            "content-addressed source {path} must use LF, not CRLF"
        )));
    }
    Ok(())
}
...
fn check_artifacts(root: &Path, artifacts: &[GeneratedArtifact]) -> Result<(), Box<dyn Error>> {
    let drift = artifacts
        ...
        .collect::<Vec<_>>();
    if drift.is_empty() {
        println!("workflow universal-assurance behavioral evidence is byte-exact");
        Ok(())
    } else {
        Err(error(format!(
            "workflow universal-assurance evidence drift:\n{}\nrun the generator with --write",
            drift.join("\n")
        )))
    }
}
...
mod tests {
    ...
    #[test]
    fn checked_in_evidence_recomputes_as_exact_closed_one_by_seven_corpus() {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
        ...
        assert!(
            artifacts
                ...
                .filter(|artifact| artifact
                    .relative_path
                    .contains("ablated-universal-assurance"))
                .count()
                == 1
        );
        assert!(
