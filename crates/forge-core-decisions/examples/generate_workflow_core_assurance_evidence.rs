use forge_core_contracts::{
    ...
    WorkflowGovernancePolicy, WorkflowGovernanceReceiptDocument, WorkflowGovernedOutcome,
...
const COVERAGE_PATH: &str = "contracts/policies/workflow-behavioral-coverage-v0.yaml";
...
const ADVERSARIAL_PATH: &str = "contracts/evidence/workflow-core-assurance-adversarial-v0.yaml";
const CORPUS_SET_PATH: &str = "contracts/evidence/workflow-core-assurance-corpus-set-v0.yaml";
const SHADOW_REPORT_PATH: &str = "contracts/evidence/workflow-core-assurance-shadow-report-v0.yaml";
const WORKFLOW_IDS: [&str; 5] = [
    "adversarial-review",
    "code-review",
    "nfr-evidence-audit",
    "risk-register",
...
fn parse_mode() -> Result<Mode, Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        _ => Err(error(
            "usage: cargo run -p forge-core-decisions --example generate_workflow_core_assurance_evidence -- (--write|--check)",
        )),
    }
}
...
fn generate(root: &Path) -> Result<Vec<GeneratedArtifact>, Box<dyn Error>> {
    ...
    let corpus_set = WorkflowBehavioralCorpusSetDocument {
        ...
        workflow_behavioral_corpus_set: WorkflowBehavioralCorpusSet {
            id: id("corpus-set.workflow-core-assurance-v0"),
    ...
    let corpus_set_artifact = artifact(
        "corpus-set.workflow-core-assurance-v0",
        CORPUS_SET_PATH,
        &corpus_set_bytes,
    );
    context.sources.insert(
    ...
    let report = evaluate_workflow_behavior(
        &WorkflowBehavioralReportIdentity {
            report_id: id("report.workflow-core-assurance.shadow-v0"),
            ...
            })
    {
        return Err(error(format!(
            "derived shadow evidence is not a closed 5x7 review candidate: {:#?}",
...
fn load_context(root: &Path) -> Result<EvidenceContext, Box<dyn Error>> {
    ...
    if subject_state.runtime_bundle.bundle_digest != runtime_digest
        ...
        || subject_state.baseline_release.release_id != baseline.active_release_id
        || subject_state.baseline_release.release_digest != baseline.active_release_digest
        || subject_state.baseline_runtime_bundle.bundle_id != baseline.runtime_bundle_id
        || subject_state.baseline_runtime_bundle.bundle_digest != baseline.runtime_bundle_digest
    {
        return Err(error(
    ...
    Ok(EvidenceContext {
        ...
        coverage_artifact: artifact(
            "coverage.workflow-core-assurance-v0",
            COVERAGE_PATH,
            &coverage_bytes,
        ),
...
fn parse_frozen_baseline(bytes: &[u8]) -> Result<FrozenBaseline, Box<dyn Error>> {
    ...
    if records.len() != 2 {
        return Err(error(
            "frozen P5d.2 history must contain exactly two records",
        ));
    }
    let imported_record = &records[0].workflow_governance_receipt;
    let upgraded_record = &records[1].workflow_governance_receipt;
    ...
    if upgraded_record.previous_record_digest.as_deref()
        != Some(imported_record.record_digest.as_str())
        || upgraded.prior_ledger_head_digest != imported_record.record_digest
        || upgraded.admission_proof.snapshot_digest != imported.snapshot_digest
    {
        return Err(error("frozen P5d.2 history digest chain is invalid"));
    }
    Ok(FrozenBaseline {
...
fn coverage_policy() -> WorkflowBehavioralCoveragePolicyDocument {
    WorkflowBehavioralCoveragePolicyDocument {
        ...
        workflow_behavioral_coverage_policy: WorkflowBehavioralCoveragePolicy {
            id: id("coverage.workflow-core-assurance-v0"),
...
fn resume_scenario(
    ...
    let checkpoint_source = artifact(
        &format!(
            "checkpoint.workflow-core-assurance.{}.v0",
...
fn ablated_path(workflow_id: &str) -> String {
    format!("contracts/workflow-governance/ablated-core-assurance-{workflow_id}-v0.yaml")
}

fn checkpoint_path(workflow_id: &str) -> String {
    format!("contracts/evidence/workflow-core-assurance-{workflow_id}-resume-checkpoint-v0.yaml")
}
...
fn check_artifacts(root: &Path, artifacts: &[GeneratedArtifact]) -> Result<(), Box<dyn Error>> {
    let drift = artifacts
        ...
        .collect::<Vec<_>>();
    if drift.is_empty() {
        println!("workflow core-assurance behavioral evidence is byte-exact");
        Ok(())
    } else {
        Err(error(format!(
            "workflow core-assurance evidence drift:\n{}\nrun the generator with --write",
            drift.join("\n")
        )))
    }
}
...
mod tests {
    ...
    #[test]
    fn checked_in_evidence_recomputes_as_exact_closed_five_by_seven_corpus() {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
        ...
        assert!(
            artifacts
                .iter()
                .filter(|artifact| artifact.relative_path.contains("ablated-core-assurance"))
                .count()
                == 5
        );
        assert!(
