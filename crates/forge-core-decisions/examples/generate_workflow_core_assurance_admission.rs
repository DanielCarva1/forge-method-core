...
use forge_core_contracts::{
    ...
    WorkflowReceiptCarryover, WorkflowReleaseAdmissionSignatureAlgorithm,
    WorkflowReleasePredecessorReference, WorkflowReleaseRegistryAuthority,
    WorkflowReleaseRegistrySource, WorkflowReleaseReviewArtifactBinding,
    ...
    WorkflowReleaseReviewerRole, WorkflowRuntimeBundleIdentity, WorkflowRuntimeBundleReference,
...
use forge_core_decisions::{
    ...
    WorkflowReleaseAdmissionCandidateInput, WorkflowReleaseAdmissionEvaluation,
...
const REVIEW_INDEX: &str = "contracts/migration/workflow-core-assurance-review-index-v0.yaml";
const REVIEWER_REGISTRY: &str = "contracts/policies/workflow-release-reviewer-registry-v0.yaml";
const INDEPENDENT_REVIEW: &str =
    "contracts/evidence/workflow-core-assurance-independent-review-v0.yaml";
const REVIEW_SIGNED_AT_UNIX: u64 = 1_783_864_800;
...
struct IndependentReviewDocument<'a> {
    ...
    workflow_release_admission_evaluation: &'a WorkflowReleaseAdmissionEvaluation,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let artifacts = vec![
        ...
        GeneratedArtifact {
            ...
            bytes: yaml_bytes(&reviewer_registry),
        },
        GeneratedArtifact {
            relative_path: REVIEW_INDEX,
            bytes: yaml_bytes(&input.review_index),
        },
        GeneratedArtifact {
...
fn parse_mode() -> Result<Mode, Box<dyn std::error::Error>> {
    ...
    match args.as_slice() {
        ...
        _ => Err("usage: cargo run -p forge-core-decisions --example generate_workflow_core_assurance_admission -- (--write|--check)".into()),
    }
}
...
fn reviewer_registry() -> WorkflowReleaseReviewerRegistryDocument {
    WorkflowReleaseReviewerRegistryDocument {
        ...
        workflow_release_reviewer_registry: WorkflowReleaseReviewerRegistry {
            registry_id: id("workflow-release-reviewers.core-assurance-v0"),
            ...
            credentials: vec![
                WorkflowReleaseReviewerCredential {
                    credential_id: id("reviewer.workflow-release.semantic-v0"),
                    principal_id: PrincipalId("principal.release-review.semantic-v0".to_owned()),
                    public_key_fingerprint:
                        "sha256:11f3e99744e5f285407f557153bbc5940e10ac9e7bb5de5c20ce599dac48771c"
                            .to_owned(),
                    public_key_hex:
                        "7167996e514536731817ce469d38eb4579e922a7c4cfafd2137a47f568c4d473"
                            .to_owned(),
                ...
                WorkflowReleaseReviewerCredential {
                    credential_id: id("reviewer.workflow-release.authorizer-v0"),
                    principal_id: PrincipalId("principal.release-review.authorizer-v0".to_owned()),
                    public_key_fingerprint:
                        "sha256:fd542a167ddcfb2fcadac9519c063f4c2b08778473591b7242045c45a9af0f18"
                            .to_owned(),
                    public_key_hex:
                        "574533c060cfc6b95f524232099c6926a8b0d55c575b427b892204ad83b8388b"
                            .to_owned(),
                    algorithm: WorkflowReleaseAdmissionSignatureAlgorithm::Ed25519,
