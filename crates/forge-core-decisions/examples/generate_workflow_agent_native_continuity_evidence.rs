...
fn parse_mode() -> Result<Mode, Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        _ => Err(error(
            "usage: cargo run -p forge-core-decisions --example generate_workflow_agent_native_continuity_evidence -- (--write|--check)",
        )),
    }
}
...
fn generate(root: &Path) -> Result<Vec<GeneratedArtifact>, Box<dyn Error>> {
    ...
    let corpus_set = WorkflowBehavioralCorpusSetDocument {
        ...
        workflow_behavioral_corpus_set: WorkflowBehavioralCorpusSet {
            id: id("corpus-set.workflow-agent-native-continuity-v0"),
    ...
    let corpus_set_artifact = artifact(
        "corpus-set.workflow-agent-native-continuity-v0",
        CORPUS_SET_PATH,
        &corpus_set_bytes,
    );
    context.sources.insert(
    ...
    let report = evaluate_workflow_behavior(
        &WorkflowBehavioralReportIdentity {
            report_id: id("report.workflow-agent-native-continuity.shadow-v0"),
            ...
            })
    {
        return Err(error(format!(
            "derived shadow evidence is not a closed 9x7 review candidate: {:#?}",
...
fn load_context(root: &Path) -> Result<EvidenceContext, Box<dyn Error>> {
    ...
    Ok(EvidenceContext {
        ...
        coverage_artifact: artifact(
            "coverage.workflow-agent-native-continuity-v0",
            COVERAGE_PATH,
            &coverage_bytes,
        ),
...
fn parse_frozen_baseline(bytes: &[u8]) -> Result<FrozenBaseline, Box<dyn Error>> {
    ...
    if records.len() != 4 {
        return Err(error(
            "frozen predecessor history must contain exactly four records",
        ));
    }
    let imported_record = &records[0].workflow_governance_receipt;
    let foundation_record = &records[1].workflow_governance_receipt;
    let core_assurance_record = &records[2].workflow_governance_receipt;
    let upgraded_record = &records[3].workflow_governance_receipt;
    ...
    if foundation_record.previous_record_digest.as_deref()
        ...
        || core_assurance_record.previous_record_digest.as_deref()
            != Some(foundation_record.record_digest.as_str())
        || upgraded_record.previous_record_digest.as_deref()
            != Some(core_assurance_record.record_digest.as_str())
        || upgraded.prior_ledger_head_digest != core_assurance_record.record_digest
...
fn coverage_policy() -> WorkflowBehavioralCoveragePolicyDocument {
    WorkflowBehavioralCoveragePolicyDocument {
        ...
        workflow_behavioral_coverage_policy: WorkflowBehavioralCoveragePolicy {
            id: id("coverage.workflow-agent-native-continuity-v0"),
...
fn resume_scenario(
    ...
    let checkpoint_source = artifact(
        &format!(
            "checkpoint.workflow-agent-native-continuity.{}.v0",
...
fn ablated_path(workflow_id: &str) -> String {
    format!("contracts/workflow-governance/ablated-agent-native-continuity-{workflow_id}-v0.yaml")
}

fn checkpoint_path(workflow_id: &str) -> String {
    format!(
        "contracts/evidence/workflow-agent-native-continuity-{workflow_id}-resume-checkpoint-v0.yaml"
    )
}
...
fn check_artifacts(root: &Path, artifacts: &[GeneratedArtifact]) -> Result<(), Box<dyn Error>> {
    let drift = artifacts
        ...
        .collect::<Vec<_>>();
    if drift.is_empty() {
        println!("workflow agent-native-continuity behavioral evidence is byte-exact");
        Ok(())
    } else {
        Err(error(format!(
            "workflow agent-native-continuity evidence drift:\n{}\nrun the generator with --write",
            drift.join("\n")
        )))
    }
}
...
mod tests {
    ...
    #[test]
    fn checked_in_evidence_recomputes_as_exact_closed_nine_by_seven_corpus() {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
        ...
        assert!(
            artifacts
                ...
                .filter(|artifact| artifact
                    .relative_path
                    .contains("ablated-agent-native-continuity"))
                .count()
                == 9
        );
        assert!(
