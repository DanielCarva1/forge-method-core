...
const BASE_BUNDLE_PATH: &str = "contracts/workflow-governance/runtime-core-assurance-v0.yaml";
...
const OVERLAY_WORKFLOW_IDS: [&str; 13] = [
    "investigation",
    "platform-ops-plan",
    "security-plan",
    "privacy-data-plan",
...
fn parse_mode() -> Result<Mode, Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        _ => Err(error(
            "usage: cargo run -p forge-core-decisions --example generate_workflow_assurance_operations_candidate -- (--write|--check)",
        )),
    }
}
...
fn generate(root: &Path, mode: Mode) -> Result<Vec<GeneratedArtifact>, Box<dyn Error>> {
    ...
    let runtime_bundle = WorkflowGovernanceBundleDocument {
        ...
        workflow_governance_bundle: WorkflowGovernanceBundle {
            id: id("bundle.workflow-governance.assurance-operations-candidate-v0"),
    ...
    let release = evaluate_workflow_release(
        &manifest,
        &[golden_batch, base_batch, batch.clone()],
        &migration_audit,
        &workflows,
    );
...
fn validate_overlay_header(
    ...
    {
        return Err(error(
            "assurance-operations overlay header or exact predecessor binding is invalid",
        ));
    }
    Ok(())
}
...
fn validate_composition(policies: &[WorkflowGovernancePolicy]) -> Result<(), Box<dyn Error>> {
    if policies.len() != 33 {
        return Err(error(format!(
            "candidate runtime must contain exactly 33 policies, found {}",
            policies.len()
        )));
    }
    ...
    let workflow_ids = policies
        ...
        .map(|policy| policy.compatibility_workflow_id.0.as_str())
        .collect::<BTreeSet<_>>();
    if policy_ids.len() != 33 || workflow_ids.len() != 33 {
        return Err(error(
...
fn validate_registry_sentinel(root: &Path) -> Result<(), Box<dyn Error>> {
    let registry_bytes = std::fs::read(root.join(PREDECESSOR_REGISTRY_PATH)).map_err(|cause| {
        error(format!(
            "embedded core-assurance registry is unavailable at {PREDECESSOR_REGISTRY_PATH}: {cause}"
        ))
    })?;
    let found = sha256(&registry_bytes);
    if found != PREDECESSOR_REGISTRY_SHA256 {
        return Err(error(format!(
            "assurance-operations generation cannot change admitted predecessor registry bytes: expected {PREDECESSOR_REGISTRY_SHA256}, found {found}"
        )));
    }
    Ok(())
}
...
fn validate_predecessor_inputs(
    ...
    if release.release_id.0 != "workflow-governance.release.core-assurance-v0"
        || release.release_version != "0.2.0"
        || release.batches.len() != 2
        || release.batches[1].embedded_ref.0 != BASE_BATCH_PATH
        || release.batches[1].expected_digest
            != workflow_migration_batch_digest(batch).map_err(error)?
        || predecessor_batch.id.0 != "workflow-governance.batch.core-assurance-v0"
        || predecessor_batch.policies.len() != 5
        || release.workflow_entries.len() != 110
    {
        return Err(error(
            "P5d.4a artifacts are not the exact expected predecessor",
        ));
    }
...
fn build_predecessor_history(root: &Path) -> Result<Vec<u8>, Box<dyn Error>> {
    let foundation_bytes = std::fs::read(root.join(FOUNDATION_HISTORY_PATH))?;
    let mut documents = std::str::from_utf8(&foundation_bytes)?
        .lines()
        .filter(|line| !line.trim().is_empty())
        .map(serde_json::from_str::<WorkflowGovernanceReceiptDocument>)
        .collect::<Result<Vec<_>, _>>()?;
    if documents.len() != 2 {
        return Err(error("foundation history must contain exactly two records"));
    }
    ...
    let previous = documents[1].workflow_governance_receipt.clone();
    let WorkflowGovernanceEvent::ReleaseUpgraded(previous_upgrade) = &previous.event else {
        return Err(error("foundation history must end in a release upgrade"));
    };
    ...
    upgrade.admission_proof.proof_id =
        id("proof.workflow-governance.release-admission.core-assurance-v0");
    upgrade
        .admission_proof
    ...
    record.record_id = id("wglr-p5d4a-core-assurance-frozen-history");
    record.sequence = 3;
...
fn build_review_subject(
    ...
    let predecessor = predecessor_registry
        ...
        .last()
        .ok_or_else(|| error("predecessor registry must contain a release"))?;
    if predecessor.release.release_id.0 != "workflow-governance.release.core-assurance-v0"
        || predecessor.runtime_bundle.identity.bundle_id.0
            != "bundle.workflow-governance.core-assurance-v0"
    {
        return Err(error("predecessor registry tail is not core-assurance"));
    }
    ...
    Ok(WorkflowBehavioralReviewSubjectDocument {
        ...
        workflow_behavioral_review_subject: WorkflowBehavioralReviewSubject {
            id: id("workflow-review-subject.assurance-operations-v0"),
            authority: WorkflowBehavioralReviewSubjectAuthority::CandidateOnly,
            overlay: WorkflowBehavioralArtifactReference {
                id: id("overlay.workflow-governance.assurance-operations-v0"),
            ...
            baseline_history: WorkflowBehavioralArtifactReference {
                id: id("history.workflow-governance.p5d2-foundation-compatibility-v0"),
            ...
            proposed_batch: WorkflowBehavioralProposedBatchSubject {
                batch_id: id("workflow-governance.batch.assurance-operations-v0"),
                batch_version: "0.3.0".to_owned(),
            ...
            proposed_release: WorkflowBehavioralProposedReleaseSubject {
                ...
                release_id: id("workflow-governance.release.assurance-operations-v0"),
...
fn build_overlay_batch(
    ...
    Ok(WorkflowMigrationBatchDocument {
        ...
        workflow_migration_batch: WorkflowMigrationBatch {
            id: id("workflow-governance.batch.assurance-operations-v0"),
            batch_version: "0.3.0".to_owned(),
...
fn build_candidate_manifest(
    ...
    release.release_id = id("workflow-governance.release.assurance-operations-v0");
    "0.3.0".clone_into(&mut release.release_version);
    ...
    [
        (
            ...
            WorkflowQuarantine {
                ...
                review_due_release_version: "0.4.0".to_owned(),
            },
        ),
        (
            "release-readiness",
...
fn validate_final_invariants(
    ...
    if quarantined != expected_quarantine
        || batch.workflow_migration_batch.policies.len() != 13
        || batch.workflow_migration_batch.workflow_bindings.len() != 13
        || runtime.workflow_governance_bundle.policies.len() != 33
        || release.batches.len() != 3
        || release.batches[0].embedded_ref.0 != GOLDEN_BATCH_PATH
        || release.batches[1].embedded_ref.0 != BASE_BATCH_PATH
        || release.batches[2].embedded_ref.0 != BATCH_PATH
    {
        return Err(error(
...
fn check_artifacts(root: &Path, artifacts: &[GeneratedArtifact]) -> Result<(), Box<dyn Error>> {
    let drift = artifacts
        ...
        .collect::<Vec<_>>();
    if drift.is_empty() {
        println!("workflow assurance-operations candidate artifacts are byte-exact");
        Ok(())
    } else {
        Err(error(format!(
            "workflow assurance-operations candidate drift:\n{}\nrun the generator with --write",
            drift.join("\n")
        )))
    }
}
...
mod tests {
    ...
    fn overlay_evaluators_have_only_authorizable_tuples() {
        ...
        .expect("exact thirteen overlay ids");
        validate_evaluator_authority_matrix(&overlay.workflow_governance_policy_overlay.policies)
    ...
    fn composed_runtime_is_valid_and_quarantine_cannot_leak() {
        ...
        assert_eq!(runtime.workflow_governance_bundle.policies.len(), 33);
        ...
        validate_composition(&runtime.workflow_governance_bundle.policies)
            .expect("closed 33-policy composition");
