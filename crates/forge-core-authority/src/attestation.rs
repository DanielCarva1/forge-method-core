...
//!
//! A transport may carry the proof in its request body (the MCP adapter uses
//! `_meta.attestation`). This crate has no MCP dependency: it verifies a
//! detached ed25519 signature over an adapter-neutral tool-call intent:
//!
//! ```text
...
//! caller-supplied key or make a nonce single-use. Mutating calls must use the
//! operator-owned [`crate::principal_registry`] binding and, at the kernel
...
const ATTESTATION_DIGEST_DOMAIN: &[u8] = b"forge-method:verified-attestation:v1\0";
const SIGNATURE_FINGERPRINT_DOMAIN: &[u8] = b"forge-method:attestation-signature:v1\0";
...
pub struct CanonicalIntent {
    /// The adapter command/tool name being called.
    pub tool: String,
    /// The exact structured arguments supplied to the adapter.
    pub arguments: Value,
...
/// Untrusted wire attestation material. The MCP adapter currently carries this
/// in `_meta.attestation`; other adapters may project the same fields. The
...
pub struct AttestationInput {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub audience: Option<String>,
    /// Optional P4a execution-intent digest, signed into the adapter call.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub execution_intent_digest: Option<String>,
...
/// The outcome of an adapter attestation gate (ADR-0006 Decision 4). `None`
/// means the gate passed (attestation present+valid, OR not
...
/// Build pseudonymous audit fingerprints only after signature verification
/// against an operator-selected key succeeds.
pub(crate) fn attestation_fingerprints(
    intent: &CanonicalIntent,
    attestation: &AttestationInput,
) -> Result<(String, String), AttestationError> {
    let canonical = intent.canonical_bytes()?;
    let signature = hex_decode(&attestation.signature)
        .map_err(|error| AttestationError::SignatureDecode(error.to_string()))?;
    if signature.len() != 64 {
        return Err(AttestationError::SignatureDecode(format!(
            "expected 64 bytes, got {}",
            signature.len()
    ...
    attestation_hasher.update(ATTESTATION_DIGEST_DOMAIN);
    update_length_prefixed(&mut attestation_hasher, &canonical);
    update_length_prefixed(&mut attestation_hasher, &signature);

    let mut signature_hasher = Sha256::new();
    signature_hasher.update(SIGNATURE_FINGERPRINT_DOMAIN);
    update_length_prefixed(&mut signature_hasher, &signature);

    Ok((
        format!("sha256:{:x}", attestation_hasher.finalize()),
        format!("sha256:{:x}", signature_hasher.finalize()),
    ))
}
