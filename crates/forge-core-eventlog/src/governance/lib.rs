...
impl ArbitrationProjection {
    ...
    pub fn apply_event(&mut self, event: &GovernanceEvent) {
        apply_event::<GovernanceDomain>(self, event);
    }
...
pub(crate) struct GovernanceDomain;
...
/// entry points retain their private stream transaction; callers must not
...
pub fn project(root: impl AsRef<Path>) -> ProjectionResult {
    let mut transaction =
        StreamTxn::begin::<ArbitrationProjectionDiagnostic>(root.as_ref(), StreamId::Governance)?;
    transaction.project::<GovernanceDomain>()
}
...
mod tests {
    ...
    fn next_sequence_starts_at_one_and_exhausts() {
        ...
        assert_eq!(next_sequence::<GovernanceDomain>(&empty), Ok(1));
