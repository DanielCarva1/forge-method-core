...
use super::{ForgetError, MemoryDomain, MemoryEvent, MemoryProjection, MemoryProjectionDiagnostic};
...
pub fn forget_with_durability(
    ...
    let mut transaction =
        match StreamTxn::begin::<MemoryProjectionDiagnostic>(root, StreamId::Memory) {
    ...
    let projection = match transaction.project::<MemoryDomain>() {
    ...
    let sequence = match next_sequence::<MemoryDomain>(&projection) {
    ...
    match transaction.append::<MemoryDomain>(&event, durability) {
        Ok(()) => ForgetResult {
...
pub fn list_now_with_durability(
    ...
    let newly_stale: Vec<MemoryEntry> = contract_doc
        .memory_contract
        .entries
        .iter()
        .filter(|entry| {
            entry.freshness.stale
                && projection
                    .entries
                    .get(&entry.entry_id.0)
                    .is_some_and(|original| !original.freshness.stale)
        })
        .cloned()
        .collect();
    let flipped = newly_stale.len();

    // Persist every new flip in order. A failed append or requested sync has
    // an indeterminate on-disk outcome, so stop immediately and never claim a
    // completed sweep (nor attempt a duplicate sequence on a later entry).
    let mut sequence = projection.sequence;
    for entry in newly_stale {
        sequence = match sequence.checked_add(1) {
            Some(sequence) => sequence,
            None => {
                return ListResult {
                    status: ListStatus::StoreError(crate::EventLogError::SequenceExhausted),
                };
            }
        };
        ...
        if let Err(source) = transaction.append::<MemoryDomain>(&event, durability) {
            return ListResult {
                status: ListStatus::StoreError(source),
            };
        }
...
#[cfg(test)]
mod tests {
    use super::super::{admit, project, AdmissionStatus};
    use super::*;
    use forge_core_contracts::{
