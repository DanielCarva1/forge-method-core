...
impl MemoryProjection {
    ...
    pub fn apply_event(&mut self, event: &MemoryEvent) {
        apply_event::<MemoryDomain>(self, event);
    }
...
/// PEP entry points retain their private stream transaction; re-acquiring it would self-deadlock.
...
pub fn project(root: impl AsRef<Path>) -> ProjectionResult {
    let mut transaction =
        StreamTxn::begin::<MemoryProjectionDiagnostic>(root.as_ref(), StreamId::Memory)?;
    transaction.project::<MemoryDomain>()
}
...
mod tests {
    ...
    fn next_sequence_starts_at_one_and_exhausts() {
        ...
        assert_eq!(next_sequence::<MemoryDomain>(&empty), Ok(1));
