...
#[cfg(test)]
use crate::tcb::{next_sequence, replay};
use crate::{
    event_envelope,
    tcb::{apply_event, EventSourced, StreamId, StreamTxn},
    CODE_OUT_OF_ORDER_EVENT_IGNORED, CODE_TORN_FINAL_LINE_SKIPPED,
};
use forge_core_contracts::ResearchSource;
use schemars::JsonSchema;
use serde::{Deserialize, Serialize};

/// Compatibility re-export of the non-authoritative clock helper.
...
impl ResearchProjection {
    ...
    pub fn apply_event(&mut self, event: &ResearchEvent) {
        apply_event::<ResearchDomain>(self, event);
    }
...
pub(crate) struct ResearchDomain;
...
impl EventSourced for ResearchDomain {
    ...
    fn diagnostic_out_of_order_event_ignored(
        ...
        ResearchProjectionDiagnostic {
            ...
            code: CODE_OUT_OF_ORDER_EVENT_IGNORED.into(),
            message: format!(
    ...
    fn diagnostic_torn_final_line_skipped(
        ...
        ResearchProjectionDiagnostic {
            severity: ResearchProjectionSeverity::Warning,
            code: CODE_TORN_FINAL_LINE_SKIPPED.into(),
...
/// entry point: `admit_source`) retains its private stream transaction
...
pub fn project(root: impl AsRef<Path>) -> ProjectionResult {
    let mut transaction =
        StreamTxn::begin::<ResearchProjectionDiagnostic>(root.as_ref(), StreamId::Research)?;
    transaction.project::<ResearchDomain>()
}
...
mod tests {
    ...
    #[test]
    fn next_sequence_starts_at_one_and_exhausts() {
        let empty = ResearchProjection::default();
        assert_eq!(next_sequence::<ResearchDomain>(&empty), Ok(1));
        ...
        assert!(matches!(
            ...
            Err(crate::EventLogError::SequenceExhausted)
        ));
    }
    ...
    fn project_hard_fails_on_schema_drift_line() {
        ...
        assert!(
            matches!(err, crate::EventLogError::Parse { line_number: 1, .. }),
