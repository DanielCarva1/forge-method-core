...
use super::{ResearchAdmitError, ResearchDomain, ResearchEvent, ResearchProjectionDiagnostic};
...
pub fn admit_source_with_durability(
    ...
    let mut transaction =
        match StreamTxn::begin::<ResearchProjectionDiagnostic>(root, StreamId::Research) {
    ...
    let projection = match transaction.project::<ResearchDomain>() {
    ...
    let sequence = match next_sequence::<ResearchDomain>(&projection) {
    ...
    match transaction.append::<ResearchDomain>(&event, durability) {
        Ok(()) => AdmissionResult {
...
#[cfg(test)]
mod tests {
    use super::super::{project, RESEARCH_LOG_RELATIVE_PATH};
    use super::*;
    ...
    #[test]
    fn admit_denied_by_gate_appends_no_event() {
        ...
        // A pure-PDP denial occurs before transaction creation, so no stream
        // file exists and no sequence is consumed.
