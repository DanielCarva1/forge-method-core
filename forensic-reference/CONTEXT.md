...
The default read-only intelligent entry operation for borrowed-shell agents. `guide` orchestrates the next protocol step from human input, Forge state, contracts, gates, and host context. It may recommend `oracle`, `gate`, `doctor`, or an explicit mutating operation, but it must not mutate state directly or become a generic "do whatever is next" command.

## Read Operation

A non-mutating Forge operation that helps the agent understand, route, validate, or diagnose before doing work. Canonical read operations include `guide`, `oracle`, `gate`, and `doctor`. They exist so agents can be autonomous in reasoning while Forge keeps mutation authority explicit.

## Explicit Mutating Operation

A named Forge write operation with a narrow side effect, such as `apply_transition`, `record_artifact`, `record_evidence`, `record_decision`, `claim_lane`, `release_lane`, or `update_heartbeat`. The Rust core must not expose a generic `advance` operation because generic continuation lets host agents smuggle their own route decisions into Forge state.

## Agent-Facing Documentation

Any Forge document whose primary reader is an agent, even when the purpose is to produce a better human conversation. Forge assumes humans experience the method through the agent; durable docs should therefore be optimized for agent recovery, routing, behavior, validation, and handoff rather than long direct human reading.

## Protocol Contract

A typed Forge contract that defines what an agent may do, must not do, and must preserve for runtime behavior. Protocol Contracts cover operation mode, authority source, state transitions, side effects, allowed actions, forbidden actions, gates, commands, and done or stop conditions.

## Behavior Contract

A compact agent-facing contract that defines how the agent should guide a human for a given mode or workflow. It captures tone, energy matching, facilitation moves, first question, escalation, humor/friction boundaries, and when to slow down or accelerate without using long prompt-like prose.

## Knowledge Contract

A compact source-of-truth contract for facts, decisions, terminology, architecture boundaries, product context, conventions, and constraints that agents need to reason correctly. Knowledge Contracts are not route authority unless a Protocol Contract explicitly consumes them.

## Markdown Render

A human-readable rendering generated from, or subordinate to, typed Forge contracts. Markdown Render is allowed for README, installation instructions, and release notes. ADR/design Markdown is temporary planning material only and must not become runtime or agent-facing authority. Markdown is not authoritative for runtime route, state, workflow, behavior, side effect, operation mode, gates, recovery, or commands.

## Forge Core

The Rust runtime authority layer responsible for typed protocol contracts, state machine transitions, validation, ledger events, recovery contracts, and command side-effect policy. Forge Core is not a Python script layer; Python may exist only as historical reference while the new core is designed and rewritten.

## Rust-Only Core Rule

The architectural rule that new Forge Core implementation work must be Rust-first. Python is not an allowed implementation target for new core runtime authority, state machine, guidance, gates, stores, contracts, lanes, locks, or host operation surfaces. Claims that Rust cannot support Codex, Cursor, Claude, OpenCode, VS Code, or CLI adapters require proof, not assumption.

## Forge Adapter

A client-specific integration layer for Codex, Cursor, Claude, OpenCode, VS Code, pi.dev, CLI, or another host. Adapters translate Forge Core contracts into the host agent surface and must not become independent route authorities.

## Borrowed Shell Runtime

The current Forge Core product shape: a Rust method engine that runs inside or beside host tools such as Codex, Cursor, Claude, OpenCode, VS Code, pi.dev, and CLI. The host provides the chat/editor/tooling shell; Forge provides state, contracts, guidance, gates, authority, and installable operation surfaces. Borrowed Shell Runtime is not the standalone Forge app.

## Host Surface

A Rust-native way for a host to call Forge Core, such as CLI JSON, MCP tools, HTTP/SSE, extension command protocol, or typed file contracts. Host Surfaces expose Forge operations without moving method logic into host-specific prompts.

## CLI JSON Surface

The universal Forge Host Surface for command-line and extension integration. It reads and writes structured JSON, is easy to test in isolation, and must call the same Rust Core operations as every other surface.

## MCP Surface

The Forge Host Surface for agent clients that support the Model Context Protocol. It exposes a small macro-tool set over the same Rust Core operations as CLI JSON and must not contain independent method logic.

## Host Package

An installable wrapper for one host surface, such as a Codex plugin, Cursor extension, OpenCode integration, VS Code extension, pi.dev integration, MCP package, or CLI binary package. Host Packages may include thin instructions and manifests, but must not own Forge method behavior.

## Forge Contract

A versioned YAML or JSON contract consumed by Forge Core or adapters. Forge Contracts are schema-validated, compact, and designed for Rust/Serde as the implementation baseline.

## Forge Renderer

A component or script that renders typed Forge Contracts into Markdown or another human-readable/exportable format. Renderers do not own runtime authority.

## Runtime Authority

The right to change Forge state, route, phase, workflow, next action, artifact lifecycle, or side-effect permission. Runtime Authority belongs to explicit Forge Core contracts, workflow gates, durable state, or clear human decisions, not to freehand agent prose.

## Authority Evidence

A compact reference proving where Runtime Authority came from, such as a human decision digest, gate result, state-machine rule, lane claim, or system maintenance policy. An Operation Contract carries Authority Evidence but is not itself an authority source.

## Funnel Autonomy

The Forge autonomy gradient: early work is highly guided, conversational, question-heavy, research-aware, and correction-friendly; later mechanical development becomes increasingly autonomous after decisions, contracts, stories, and gates are settled. Funnel Autonomy lets the agent explore and propose broadly while requiring explicit authority for state mutation, route changes, and release decisions.
...
The default Forge behavior for procedural work after decision phases are settled. It lets the agent create stories, implement, review, repair, test, write evidence, update sprint state, and move readiness forward without asking for procedural confirmation.
