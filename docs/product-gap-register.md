...
| **GAP-002** | P0 | `engineering_hardening_debt` | **A missing linked sidecar can be recreated instead of being treated as possible durable-state loss.** This can replace lost ledgers/receipts with fresh state and blur recovery with reinitialization. There is no product-owned typed backup/restore operation for the complete authority set. | `crates/forge-core-cli/src/start_cmd.rs` classifies `link_present_no_sidecar` and calls the initialization path with `repaired_sidecar`. Operator guidance currently relies on coordinated filesystem backup and manual restoration. | A link proving prior initialization plus a missing sidecar fails closed; explicit bootstrap recovery is distinguishable from restoration. Typed backup/verify/restore binds the Project Link, complete sidecar, external anchors/registries, and exact release identity, with loss and rollback tests. |
