...
| **GAP-002** | P0 | `partial_public_surface` | **Complete-state backup, verification, restore, and explicit bootstrap-recovery choices are not product-owned.** Linked sidecar loss now fails closed instead of silently replacing lost ledgers/receipts, but operators still lack a verified complete-authority recovery operation. | `crates/forge-core-cli/src/start_cmd.rs` returns a typed `linked_state_unavailable` rejection with a machine-readable loss cause, project/link identity, an explicitly unavailable workflow release identity, and zero mutation for missing, empty, partial, inaccessible, or symlink-substituted linked state. Clean no-link bootstrap remains separate. Operator guidance still relies on coordinated filesystem backup and manual recovery planning. | Explicit bootstrap recovery is distinguishable from restoration. Typed backup/verify/restore binds the Project Link, complete sidecar, external anchors/registries, and exact release identity, with omission, loss, rollback, interruption, and cross-project tests. |
