...
Basis bytes come only from exact regular-file handles in the retained project snapshot used for the admission. Absolute/traversal paths, links, files outside that snapshot, and excluded roots such as `.git`, `.forge-method`, `target`, and `node_modules` cannot become basis. The audit publishes source-satisfaction proof only for a current pass; current fail and inconclusive records publish only the kernel-verified content-addressed-basis proof. A completion binds the exact current pass record digests, and expiry, supersession, snapshot/basis drift, or another freshness loss invalidates that completion rather than falling back to an older assessment.
