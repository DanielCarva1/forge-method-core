...
`<repo-root>\docs\dev-docs\forge-method-core-dev-docs-v2\`
...
- Core resolves as bootstrap exception: `<repo-root>\.forge-method`.
- Darkest resolves through sidecar: `D:\darkest-roguelite\forge-darkest-roguelite\.forge-method`.
- Old nested `<repo-root>\darkest-roguelite\` is empty only.
- `<repo-root>\hostfully-related\` is also an empty local remnant.
