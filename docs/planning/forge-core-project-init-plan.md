...
Forge core itself remains the bootstrap exception: local state under `D:\Forge-method-core\.forge-method` is allowed only when commands explicitly opt in with `--allow-bootstrap-core`.
